...
function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[var(--gradient-soft)]">
      <header className="container mx-auto flex items-center justify-between px-6 py-6 motion-rise">
        ...
        <Link to="/auth"><Button className="pressable">Entrar</Button></Link>
      </header>

      <main className="container mx-auto px-6 pt-12 pb-24">
        <div className="mx-auto max-w-3xl text-center motion-blur-in">
          ...
          <h1 className="mt-6 font-bold tracking-tight">
            Gestão completa dos{" "}
            <span className="text-iridescent">cultos</span>{" "}
            da sua congregação
          </h1>
          <p className="mt-6 text-lg text-muted-foreground" style={{ fontSize: "clamp(1rem, 0.95rem + 0.3vw, 1.2rem)" }}>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Link to="/auth">
              <Button size="lg" className="shadow-[var(--shadow-elegant)] hover-lift pressable">
                Começar agora
              </Button>
            </Link>
          </div>
        ...
        <div className="mx-auto mt-16 grid max-w-4xl grid-cols-2 gap-3 sm:grid-cols-3">
          {features.map((f, i) => (
            <div
              key={f.label}
              className="motion-rise hover-lift flex items-center gap-3 rounded-xl glass p-4"
